...
//            add_action('activated_plugin', array($this, 'plugin_changes'), 10, 2);
        ...
        public function get_services_options($selected_value=false, $language){
            ...
            foreach($services as $service){
                if (strlen($service->name)==0) continue;

                $sel = ($selected_value == $service->name) ? 'selected="selected"' : '';
                $html .= '<option value="'.esc_html($service->name).'" '.$sel.'>'.esc_html($service->name).'</option>';
            }

            if (strlen($html)==0) {
                $html = '<option value="">'.esc_html(__('No services listed','complianz-gdpr')).'</option>';
            }
            return $html;
        }
                ...
                if ($status==200){
                    ...
                    if ($language==='en') {
                        foreach ($serviceTypes as $serviceType) {
                            if (strlen($serviceType) == 0) continue;
                            cmplz_register_translation($serviceType, $serviceType);
                        }
                    }

                    set_transient('cmplz_serviceTypes_'.$language, $serviceTypes, WEEK_IN_SECONDS);
                }
            }
            if ($serviceTypes) {
                ...
                foreach ($serviceTypes as $serviceType) {
                    ...
                    if (strlen($serviceType) == 0) continue;
                    $sel = ($selected_value == $serviceType) ? 'selected="selected"' : '';
                    $html .= '<option value="' . esc_html($serviceType) . '" ' . $sel . '>' . esc_html($serviceType) . '</option>';
                }
            }

            if (strlen($html)==0) {
                $html = '<option value="">'.esc_html(__('No service types listed','complianz-gdpr')).'</option>';
            }
            return $html;
        }
        ...
        public function get_cookiePurpose_options($selected_value=false, $language){
                ...
                if ($status==200){
                    ...
                    if ($language==='en') {
                        foreach ($cookiePurposes as $cookiePurpose) {
                            if (strlen($cookiePurpose) == 0) continue;
            ...
            if ($cookiePurposes) {
                foreach ($cookiePurposes as $cookiePurpose) {
                    ...
                    if (strlen($cookiePurpose) == 0) continue;
                    $sel = ($selected_value == $cookiePurpose) ? 'selected="selected"' : '';
                    $html .= '<option value="' . esc_html($cookiePurpose) . '" ' . $sel . '>' . esc_html($cookiePurpose) . '</option>';
                }
            }

            if (strlen($html)==0) {
                $html = '<option value="">'.esc_html(__('No purposes listed','complianz-gdpr')).'</option>';
            }
            return $html;
        }
        ...
        public function get_cookie_list_item_html($tmpl, $name, $language){
            ...
            $icons .= $this->get_icon(array(
                ...
                'icon_success' => 'check',
                'icon_error' => 'times',
                'desc_success' =>  __("The data for this cookie is complete","complianz-gdpr"),
                'desc_error' =>  __("This cookie has missing fields","complianz-gdpr"),
            ));
            if (!$cookie->sync){
                $status = 'disabled';
            ...
            $icons .= $this->get_icon(array(
                ...
                'desc_success' => __("This cookie has been synchronized with cookiedatabase.org","complianz-gdpr"),
            ...
            $icons .= $this->get_icon(array(
                'status' => $status,
                'icon_success' => 'file',
                'desc_success' =>  __("This cookie will be on your cookie policy","complianz-gdpr"),
                'desc_disabled' => __("This cookie is not shown on the cookie policy","complianz-gdpr"),
            ) );
            ...
            $icons .= $this->get_icon(array(
                ...
                'desc_success' =>  __("This cookie has recently been detected","complianz-gdpr"),
                'desc_error' =>  __("This cookie has not been detected on your site in the last three months","complianz-gdpr"),
            ));
        ...
        public function get_icon($args){
            ...
            if (!$args['icon_error']) $args['icon_error'] = $args['icon_success'];
            $complete = ($args['status']=='success') ? $args['icon_success'] : $args['icon_error'];
            switch ($args['status']){
                case 'success':
            ...
            return '<span cmplz-tooltip="'.$desc.'" flow="left"><i class="fa cmplz-tooltip-icon  cmplz-'.esc_attr($args['status']).' fa-'.$complete.'" ></i></span>';
        }
        ...
        public function get_service_list_item_html($tmpl, $name, $language){
            ...
            $icons .= $this->get_icon(array(
                ...
                'icon_success' => 'check',
                'icon_error' => 'times',
                'desc_success' =>  __("The data for this service is complete","complianz-gdpr"),
                'desc_error' =>   __("This service has missing fields","complianz-gdpr"),
            ));
            ...
            $icons .= $this->get_icon(array(
                'status' => $status,
                'icon_success' => 'sync-alt',
                'desc_success' =>  __("This service has been synchronized with cookiedatabase.org","complianz-gdpr"),
                'desc_error' =>   __("This service is not synchronized with cookiedatabase.org","complianz-gdpr"),
                'desc_disabled' =>   __("Synchronization with cookiedatabase.org is not enabled for this service","complianz-gdpr"),
            ));
        ...
        public function ajax_get_list(){
            ...
            $type='cookie';
            if (isset($_GET['type']) && in_array($_GET['type'], array('service','cookie'))) {
                $type = $_GET['type'];
            }
            ...
            if ($type=='cookie'){
                ...
                if ($grouped_by_service) {
                    foreach ($grouped_by_service as $service => $cookies) {
                        ...
                        foreach($cookies as $cookie) {
                            $html .= $this->get_cookie_list_item_html($tmpl, $cookie->name, $language);
                        }
                    }
                }
                ...
                if ($items) {
                    foreach ($items as $service => $item) {
                        $html .= $this->get_service_list_item_html($tmpl, $item->name, $language);
                    }
                }
            }
        ...
        public function ajax_edit_item(){
            ...
            $type='cookie';
            if (isset($_POST['type']) && in_array($_POST['type'], array('service','cookie'))) {
                $type = $_POST['type'];
            }
            ...
            if (!$error && $action==='save' && $item_id) {
                ...
                if (!$error) {
                    $item = ($type==='cookie') ? new CMPLZ_COOKIE($item_id) : new CMPLZ_SERVICE($item_id);

                    $data = json_decode(stripslashes($_POST['data']), true);
                    foreach ($data as $key => $value) {
                        if (!strpos($key, 'cmplz_') === FALSE) continue;
            ...
            $data = array(
                    ...
                    'action' => $action,
                    'html' => $html,
                    'divider' => '<div class="cmplz-service-divider no-service">' . __('Cookies without selected service', 'complianz-gdpr') . '</div>',
...
//		            $wpdb->prefix . 'cmplz_services'
        ...
        public function enqueue_admin_assets($hook)
        {
            ...
            if (isset($_GET['page']) && $_GET['page']=='cmplz-wizard') {
                wp_register_style('select2', cmplz_url . 'core/assets/select2/css/select2.min.css', false, cmplz_version);
                wp_enqueue_style('select2');
                wp_enqueue_script('select2', cmplz_url . "core/assets/select2/js/select2.min.js", array('jquery'), cmplz_version, true);
...
//        }
        ...
        public function run_cookie_scan()
        {
            ...
            if ($this->scan_complete() && ($one_month_ago > $last_scan_date)) {
                $this->reset_pages_list();
            }
        ...
        private function parse_for_statistics_settings($html){
            ...
            if (strpos($html, 'gtm.js')!==false || strpos($html, 'gtag/js')!==false){
                ...
                if ($matches && isset($matches[2])) {
                    cmplz_update_option('wizard', 'GTM_code', sanitize_text_field($matches[2]));
            ...
            if (strpos($html, 'analytics.js')!==false || strpos($html, 'ga.js')!==false){
                update_option('cmplz_detected_stats_type',true);
        ...
        public function get_cookies_by_service($settings = array()){
            ...
            $topServiceID = 0;
            foreach($cookies as $cookie){
                $serviceID = $cookie->serviceID ? $cookie->serviceID : 999999999;
                $topServiceID = $serviceID>$topServiceID ? $serviceID : $topServiceID;
                $purpose = strlen($cookie->purpose)==0 ? __('Purpose pending investigation', 'complianz-gdpr') : $cookie->purpose;
                $grouped_by_service[$serviceID][$purpose][] = $cookie;
            }

            //move misc to end of array
            $misc = isset($grouped_by_service[999999999]) ? $grouped_by_service[999999999] : false;
            unset($grouped_by_service[999999999]);
            if ($misc) $grouped_by_service[$topServiceID+1] = $misc;
        ...
        public function get_services($settings = array())
        {
            ...
            if ($settings['category']!=='all'){
                $sql .= $wpdb->prepare(" AND category = %s", $settings['category']);
            }
        ...
        public function get_supported_languages($count=false){
            ...
            if ($count) return count($languages);
        ...
        public function run_sync_on_update(){
            ...
            if ($attempts<3){
                ...
                $attempts = $attempts+1;
                set_transient('cmplz_sync_attempts',$attempts, DAY_IN_SECONDS);
            }
        }
        ...
        {
            if ($type === 'showOnPolicy') return true;
